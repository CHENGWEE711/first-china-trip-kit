...
公开商品仍显示 `Only -1 left` 和 `On Sale`。Payhip 管理后台在当前浏览器要求重新登录，另一登录会话被 Cloudflare 拦截；没有可用的 Payhip Connector/API，因此本轮不能安全修改后台或执行测试交付。

**唯一需要用户手动完成的事项：Payhip 后台商品设置。** 在后台关闭库存数量限制并移除 `Only -1 left`；如无真实折扣则关闭 `On Sale`；确认数字产品无限库存与价格仍为 $7；随后使用 Payhip 允许的测试方式验证数字文件交付和购买邮件。该项未被宣称已修复。
...
仅 Payhip 后台商品设置与测试交付一项，具体操作见第 7 节。网站生产集成、入口、SEO、图片、Store、GA4 与正式域名验收没有其他未完成问题。
